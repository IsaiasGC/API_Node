...
router.get('/:id', ensure.authorized, async (req, res)=>{
  ...
  router.post('/', ensure.authorized, async (req, res)=>{
    const { id, name, description, idteacher_id } = req.body;
    var val = 0
    await pool.query("insert into tblcourse(id, name, description, idteacher_id) values($1, $2, $3, $4);",
    [id, name, description, idteacher_id], (error, results) => {
      if(error){
        res.status(400).json({
          status: 'not inserted',
          valid: val
